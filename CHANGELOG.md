...
### Added
- **Autonomous Discipline Intelligence & Anti-Burnout Engine (Phase 14)**: Deterministic, privacy-preserving mathematical heuristics evaluating cognitive load, habit fatigue, velocity decay, and workload strain with explainable confidence scores and zero third-party LLM data leakage (`src/lib/discipline/insights-engine.ts`, `src/features/discipline/discipline-actions.ts`, `src/features/discipline/components/discipline-insights-card.tsx`).
- **Enterprise Single Sign-On (SSO) & OIDC/SAML2 Security Engine (Phase 14)**: Enterprise identity integration supporting OpenID Connect and SAML 2.0 metadata, single-use state challenges with 10-minute TTL, domain matching safeguards, and safe workspace role mapping (`src/lib/auth/sso-engine.ts`, `src/features/auth/sso-actions.ts`, `src/features/settings/components/sso-settings-card.tsx`).
- **Production GA Runtime Validator (Phase 14)**: Automated sanity checker validating environment variables, 26-migration Supabase catalog continuity, zero-secret scanning, and operational readiness (`src/lib/config/production-validator.ts`).
- **Discipline Intelligence & Enterprise SSO Migration (Phase 14)**: PostgreSQL migration introducing `discipline_insights`, `sso_providers`, and `sso_audit_logs` tables with Supabase Row Level Security and composite query indexes (`supabase/migrations/20260917000000_discipline_intelligence_and_enterprise_sso.sql`).
- **Phase 14 Comprehensive Regression Test Suite (Phase 14)**: Automated tests verifying deterministic heuristics, focus overextension, habit fatigue, replay protection, domain matching, and consequence confidentiality (`tests/phase14-discipline-and-enterprise-sso.test.ts`).
