...
async function runReleasePackagingTests() {
  ...
  console.log('\n2. Validating Supabase migration catalog (1 through 25)...');
  const migrationsDir = path.join(rootDir, 'supabase', 'migrations');
  assert.ok(fs.existsSync(migrationsDir), 'supabase/migrations directory must exist');
  const migrations = fs.readdirSync(migrationsDir).filter(f => f.endsWith('.sql')).sort();
  assert.strictEqual(migrations.length, 25, 'Exactly 25 sequential migrations must be present');
  ...
  console.log('✅ All 25 migrations validated: RLS enforced, sequential timestamps, non-destructive.');
