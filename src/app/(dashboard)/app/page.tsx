...
import { User, ShieldCheck, Target, FolderKanban, CheckSquare, ArrowRight } from 'lucide-react';
...
export default async function ProtectedAppPage() {
  ...
  const pendingTasksCount = tasks ? tasks.filter((t) => t.status === 'pending' || t.status === 'in_progress').length : 0;
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-zinc-100 flex flex-col">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-6 sm:p-10 flex flex-col items-center justify-center space-y-8">
        <div className="w-full glass-card p-8 rounded-3xl border border-zinc-800/80 max-w-3xl text-center space-y-6">
            ...
            <div className="bg-zinc-950/80 p-5 rounded-2xl border border-zinc-800 flex flex-col justify-between space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-400">
                    {pendingTasksCount} active {pendingTasksCount === 1 ? 'commitment' : 'commitments'}
                  </p>
                </div>
              </div>
          ...
          <div className="pt-2 text-xs text-zinc-500">
            Phase 2D Vertical Slice Operational • Real Database RLS & Double-Layer Parent Authorization Enforced
          </div>
        </div>
      </main>
    </div>
